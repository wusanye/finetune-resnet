...
def dense(x, num_outputs, activation=None, name='fc', verbose=False):
    ...
    w = create_variable(name='weights', shape=[num_inputs, num_outputs])

    b = create_variable(name='biases', shape=[num_outputs], initializer=tf.zeros_initializer())

    act = tf.nn.xw_plus_b(x, w, b, name='BiasAdd')
    ...
    if verbose:
        print("> %s layer: num_in=%d, num_out=%d, output_shape=%s" % (name, num_inputs, num_outputs, str(act.get_shape())))
...
def leaky_relu(x, alpha=0.1):
    ...
    activation = tf.nn.leaky_relu(x, alpha=alpha,name='LeakyRelu')
